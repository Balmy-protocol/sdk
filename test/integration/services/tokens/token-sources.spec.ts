...
describe('Token Sources', () => {
  ...
  tokenSourceTest({ title: 'Fallback Source', source: FALLBACK_TOKEN_SOURCE, validate: { fieldsExist: ['price', 'timestamp'], on: DEFI_LLAMA_TOKEN_SOURCE.supportedNetworks() } })

  function tokenSourceTest<T extends BaseToken>({ title, source, validate }: { title: string, source: ITokenSource<T>, validate?: { fieldsExist: (keyof T & string)[], on?: Network[] } }) {
    describe(title, () => {
      ...
      function validateToken({ network, address, symbol, decimals }: { network: Network, address: TokenAddress, symbol: string, decimals: number }) {
