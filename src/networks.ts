...
export class Networks {
  ...
  static getAllNetworks(): Network[] {
    return Object.values(Networks).filter((value): value is Network => typeof value !== 'function');
  }

  static byKey(key: string | ChainId): Network | undefined {
    const toLower = `${key}`.toLowerCase();
    return Networks.getAllNetworks().find(({ chainId, ids }) => `${chainId}` === toLower || ids.includes(toLower));
  }
