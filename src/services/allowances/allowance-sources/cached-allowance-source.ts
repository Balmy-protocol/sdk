...
export class CachedAllowanceSource implements IAllowanceSource {
  ...
  constructor(private readonly source: IAllowanceSource, config: CacheConfig) {
    this.cache = new ConcurrentLRUCache<Key, bigint>({
  ...
  private async fetchTokens(keys: Key[]): Promise<Record<Key, bigint>> {
...
function resultsToKeyResults(result: Record<ChainId, Record<TokenAddress, Record<OwnerAddress, Record<SpenderAddress, bigint>>>>) {
  const keyResults: Record<Key, bigint> = {};
...
function keyResultsToResult(keyResults: Record<Key, bigint>) {
  const result: Record<ChainId, Record<TokenAddress, Record<OwnerAddress, Record<SpenderAddress, bigint>>>> = {};
