...
function addressesToTokensInNetwork(addresses: Record<ChainId, TokenAddress[]>): TokenInNetwork[] {
  return Object.entries(addresses).flatMap(([chainId, addresses]) =>
    addresses.map(address => toTokenInNetwork(parseInt(chainId), address)))
}
...
function tokenInNetworkRecordToChainAndAddress<Token extends BaseToken>(record: Record<TokenInNetwork, Token>): Record<ChainId, Record<TokenAddress, Token>> {
...
function chainAndAddressRecordToTokenInNetwork<Token extends BaseToken>(record: Record<ChainId, Record<TokenAddress, Token>>): Record<TokenInNetwork, Token> {
  const entries = Object.entries(record)
    .flatMap(([chainId, record]) =>
      Object.entries(record)
        .map<[TokenInNetwork, Token]>(([address, token]) => [toTokenInNetwork(parseInt(chainId), address), token]))
