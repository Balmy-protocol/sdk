...
export function addQuoteSlippage(quote: SlippagelessQuote, type: 'sell' | 'buy', slippagePercentage: number): SourceQuoteResponse {
  return (type === 'sell')
    ? { ...quote, type, minBuyAmount: quote.buyAmount.sub(calculatePercentage(quote.buyAmount, slippagePercentage)), maxSellAmount: quote.sellAmount }
    : { ...quote, type, maxSellAmount: quote.sellAmount.add(calculatePercentage(quote.sellAmount, slippagePercentage)), minBuyAmount: quote.buyAmount }
}

export function isNativeWrapOrUnwrap(network: Network, sellToken: TokenAddress, buyToken: TokenAddress) {
  return !isSameAddress(sellToken, buyToken)
    && isNativeOrWToken(network, sellToken)
    && isNativeOrWToken(network, buyToken)
}
const isNativeOrWToken = (network: Network, address: TokenAddress) => isSameAddress(address, network.wToken) || isSameAddress(address, Addresses.NATIVE_TOKEN)