...
export class OdosQuoteSource extends BaseQuoteSource<OdosSupport, true, OdosConfig> {
  ...
  async quote({ fetchService }: QuoteComponents, { network, sellToken, buyToken, sellTokenData, buyTokenData, order, accounts: { takeFrom }, config: { slippagePercentage, timeout }, context }: SourceQuoteRequest<OdosSupport>): Promise<SourceQuoteResponse> {
    ...
    const { outputToken, transaction: { to, data, value, gas } }: Response = await response.json()
