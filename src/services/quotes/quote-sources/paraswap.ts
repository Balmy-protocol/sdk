...
import { NoCustomConfigQuoteSource as NoCustomConfigQuoteSource, QuoteComponents, QuoteSourceMetadata, SourceQuoteRequest, SourceQuoteResponse } from "./base"
...
export class ParaswapQuoteSource extends NoCustomConfigQuoteSource<ParaswapSupport> {
  ...
  private async getPrice(fetchService: IFetchService, { network, sellToken, sellTokenData, buyToken, buyTokenData, order, accounts: { takeFrom, recipient }, config: { timeout } }: SourceQuoteRequest<ParaswapSupport>) {
  ...
  private async getQuote(fetchService: IFetchService, { network, sellToken, sellTokenData, buyToken, buyTokenData, order, route, accounts: { takeFrom, recipient }, config: { slippagePercentage, txValidFor, timeout } }: SourceQuoteRequest<ParaswapSupport> & { route: any }) {
  ...
  private isWrapingOrUnwrapingWithWToken(network: Network, priceRoute: any) {
    return priceRoute.bestRoute?.[0]?.percent === 100
