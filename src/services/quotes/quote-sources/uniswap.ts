...
export class UniswapQuoteSource extends NoCustomConfigQuoteSource<UniswapSupport> {
  ...
  async quote({ fetchService }: QuoteComponents, { network, sellToken, buyToken, order, config: { slippagePercentage, timeout, txValidFor }, accounts: { takeFrom, recipient } }: SourceQuoteRequest<UniswapSupport>): Promise<SourceQuoteResponse> {
    ...
    let { quote: quoteAmount, methodParameters: { calldata }, gasUseEstimate } = body
    ...
    if (isBuyTokenNetworkToken) {
      ...
      const { data: multicallData } = await routerContract.populateTransaction.multicall([
        calldata,
        unwrapData
      ])
...
function calculateMinBuyAmount(type: 'sell' | 'buy', buyAmount: BigNumber, slippagePercentage: number) {
  return type === 'sell'
    ? buyAmount.sub(calculatePercentage(buyAmount, slippagePercentage))
    : buyAmount
}
