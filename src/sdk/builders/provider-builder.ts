...
import { LoadBalanceSource, LoadBalanceSourceConfig } from '@services/providers/provider-sources/load-balance-provider';
...
export type ProviderSourceInput =
  ...
  | { type: 'loadbalance'; sources: ProviderSourceInput[]; config?: LoadBalanceSourceConfig }
...
function buildSource(source?: ProviderSourceInput): IProviderSource {
  switch (source?.type) {
    ...
    case 'loadbalance':
      return new LoadBalanceSource(source.sources.map(buildSource), source.config);
    case 'prioritized':
