...
export function buildSDKWithProvider<CustomConfig extends Partial<AllSourcesConfig> = {}>({ fetch, provider, config }: {
  ...
  const providerSource = new FallbackWhenNetworkNotSupportedProviderSource([
    new SingleProviderSource(provider),
    new PublicProvidersSource()
  ])
...
function buildGasCalculatorBuilder({ gasPriceSource, multicallService }: { gasPriceSource: IGasPriceSource, multicallService: IMulticallService }) {
