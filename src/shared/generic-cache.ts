...
export class ContextlessCache<Key extends ValidKey, Value> {
  ...
  constructor({ calculate, toStorableKey, expirationConfig }: {
  ...
  async getOrCalculateSingle({ key, expirationConfig }: {
    key: Key,
  ...
  async getOrCalculate({ keys, expirationConfig }: {
    keys: Key[],
    expirationConfig?: Partial<ExpirationConfigOptions>,
  }) {
...
export class Cache<Context, Key extends ValidKey, Value> {
  ...
  constructor({ calculate, toStorableKey, expirationConfig }: CacheConfig<Context, Key, Value>) {
    ...
    const isInvalid = expirationConfig.useCachedValue !== 'always'
  ...
  async getOrCalculateSingle({ key, context, expirationConfig }: {
    key: Key,
    context: Context
    expirationConfig?: Partial<ExpirationConfigOptions>,
  }): Promise<Value | undefined> {
  ...
  async getOrCalculate({ keys, context, expirationConfig }: {
    keys: Key[],
    context: Context
    expirationConfig?: Partial<ExpirationConfigOptions>,
    ...
    const now = Date.now()
    const useCachedValue = ({ lastUpdated }: { lastUpdated: Timestamp }) => options.useCachedValue === 'always' || lastUpdated >= now - ms(options.useCachedValue.ifUnder)
    ...
    const useCachedValueIfCalculationFailed = ({ lastUpdated }: { lastUpdated: Timestamp }) => options.useCachedValueIfCalculationFailed === 'always' || lastUpdated >= nowAgain - ms(options.useCachedValueIfCalculationFailed.ifUnder)
